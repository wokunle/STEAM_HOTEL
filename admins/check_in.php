<?php
	...
	echo   '<div class="col-lg-10 col-md-10">
                <div class="table-responsive">
                    <form method="post" action="check_in_DONE.php">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th>#</th>
                            ...
                            <tbody>';
	$today=date("y-m-d");
    $query1 = "SELECT * FROM wkl_order WHERE complete = 0 AND check_in_date < NOW()";
    $result1 = mysql_query($query1) or die('SQL语句有误：'.mysql_error());
    $l_c=array('success','info','warming','danger');
    $i=0;
    while (!!$row1 = mysql_fetch_array($result1)) {
        echo '                  <tr class='.$l_c[$i].'>';
        echo '                      <td><input type="checkbox"  name="check_in_order[]" value='.$row1['order_code'].'></td>';
    ...
    echo '                  </tbody>
                        ...
                        <br>
                        <input class="btn btn-primary" type="submit" value="登记入住" name="check_in" />
                    </div>
            </div>';
